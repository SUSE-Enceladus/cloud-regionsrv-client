...
def add_hosts_entry(smt_server):
    ...
    log.debug('Modified /etc/hosts, added: {}'.format(entry))
    ...
    try:
        ...
        for extension in extensions:
            ...
            if not extension.get('free', True):
                ...
                if triplet in installed_products and is_product_removable(
                    triplet
                ):
...
def get_product_data(registration_target=None):
    if not registration_target:
        registration_target = get_current_smt()
    if not registration_target:
        log.debug('No update server set')
        return {}
...
def enable_repository(repo_name):
    ...
    if failed:
        log.error('Unable to enable repository {}'.format(repo_name))
...
def fetch_smt_data(cfg, proxies, quiet=False):
    ...
    if cfg.has_option('server', 'metadata_server'):
        ...
        log.debug(msg % metadata_url)
        try:
            response = requests.get(metadata_url, timeout=15.0, proxies=proxies)
        ...
        for attempt in range(max_attempts):
            ...
            for srv in region_servers:
                srvName = str(srv)
                if not quiet:
                    log.debug('\tUsing region server: {}'.format(srvName))
                certFile = os.path.normpath(
                    os.sep.join([cert_dir, srvName + '.pem'])
                )
                if not os.path.isfile(certFile):
                    log.debug(
                        '\tNo cert found: {} skip this server'.format(certFile)
                    )
                    continue
                try:
...
def find_equivalent_smt_server(configured_smt, known_smt_servers):
    ...
    for update_server in known_smt_servers:
        ...
        if update_server.get_ipv4() == configured_smt.get_ipv4():
            continue
        if (
            update_server.is_equivalent(configured_smt)
            and update_server.is_responsive()
        ):
...
def prepare_registry_setup(registry_fqdn, username, password):
    ...
    return (
        ...
        and set_container_engines_env_vars()
    )
...
def set_container_engines_env_vars():
    ...
    if os.path.exists(PROFILE_LOCAL_PATH):
        try:
            ...
            log.debug('Could not open %s: %s' % (PROFILE_LOCAL_PATH, error))
...
def update_bashrc(content, mode):
    ...
    try:
        ...
        log.debug('{} updated'.format(PROFILE_LOCAL_PATH))
        return True
    except OSError as error:
...
def clean_registry_auth(registry_fqdn):
    ...
    if not os.path.exists(REGISTRY_CREDENTIALS_PATH):
        log.debug('Credentials file does not exist. Nothing to do')
        return True

    config_json, preserve_status = get_registry_credentials(set_new=False)
    if (
        not config_json or config_json == {'auths': {}}
        ...
        if preserve_status is None:
            log.debug('JSON content is empty')
    ...
    try:
        ...
        if registry_fqdn:
            log.debug('Unsetting the auth entry for %s' % registry_fqdn)
        else:
            log.debug('Unsetting the auth entry based on the token')
            auth_token = _generate_registry_auth_token()
            entries = config_json.get('auths', {}).items()
            entry = [
                entry
                for entry, auth in entries
                if auth == auth_token
                and 'registry' in entry
                and 'susecloud.net' in entry
            ]
        ...
        if config_json.get('auths', {}).pop(registry_fqdn, {}):
            ...
            log.debug('Registry auth entry unset')
...
def unset_env_vars():
    ...
    if not os.path.exists(PROFILE_LOCAL_PATH):
        log.debug('%s file does not exist' % PROFILE_LOCAL_PATH)
...
def write_registries_conf(registries_conf, container_path, container_name):
    ...
    try:
        ...
        log.debug('File {} updated'.format(container_path))
        return True
    except IOError as error:
        log.debug(str(error))
        action = 'open'
...
def get_credentials_file(update_server, service_name=None):
    ...
    for entry in credential_names:
        ...
        if not cred_files:
            log.debug('No credentials entry for "{}"'.format(entry))
            continue
        if len(cred_files) > 1:
            log.warning(
                'Found multiple credentials for "{}" entry'.format(service_name)
            )
...
def get_current_smt():
    ...
    if not (
        re.search(ipv4_search.encode(), hosts)
...
def get_instance_data(config):
    ...
    the result or none."""
    instance_data = b''
    if config.has_section('instance') and config.has_option(
        ...
        if is_registered(current_smt.get_FQDN()):
            ...
            if alive:
                log.debug(
                    ...
                    '"%s"'
                    % str((current_smt.get_ipv4(), current_smt.get_ipv6()))
                )
                return current_smt
            else:
                ...
                for delay in [5, 3, 1]:
                    ...
                    if current_smt.is_responsive():
                        log.debug('No failover needed, system access recovered')
                        return current_smt
                ...
                new_target = find_equivalent_smt_server(
                    current_smt, available_servers
                )
                if new_target:
                    log.debug(
                        'Using equivalent update server: '
                        '"%s"'
                        % str((new_target.get_ipv4(), new_target.get_ipv6()))
                    )
...
def has_rmt_ipv6_access(smt):
    ...
    if not smt_ipv6 or not has_ipv6_access([smt_ipv6]):
        return False
    log.debug('Attempt to access update server over IPv6')
    ...
    try:
        ...
        log.debug('Update server not reachable over IPv6')
        return False
...
def has_nvidia_support():
    ...
    if 'NVIDIA' in pci_info.decode():
        log.debug('Instance has Nvidia support')
        return True
...
def has_region_changed(cfg):
    ...
    try:
        with open(get_framework_identifier_path()) as framework_file:
            registered_region = json.loads(framework_file.read())
    except Exception:
        return False

    if framework == registered_region.get(
        'framework'
    ) and region == registered_region.get('region'):
        return False
    ...
    for service_file in service_files:
        ...
        for entry in content:
            if entry.startswith('url'):
                if (
                    smt_server_name in entry
                    or 'plugin:/susecloud' in entry
                    or 'plugin:susecloud' in entry
                ):
                    return True
...
def import_smt_cert(smt):
    ...
    for site_path in site.getsitepackages():
        py_pack_certs = glob.glob(
            os.path.normpath(os.sep.join([site_path, 'certifi', '*.pem']))
        )
        if py_pack_certs:
            log.warning(
...
def get_profile_env_var(varname, profile_file):
    shell_command = [
        'bash',
        '-c',
        'source {0}; echo ${1}'.format(profile_file, varname),
    ]
...
def is_registry_registered(registry_server_name):
    ...
    if (
        registry_server_name in hosts.keys()
...
def is_registration_supported(cfg):
    ...
    try:
        ...
        if package_backend == 'dnf':
            log.debug('Registration for RHEL product family requested')
            log.debug('Exit after repository server hosts entry setup')
...
def set_proxy():
    ...
    if existing_http_proxy and existing_https_proxy:
        ...
        log.debug('Using proxy settings from execution environment')
        log.debug('\thttp_proxy: %s' % existing_http_proxy)
        log.debug('\thttps_proxy: %s' % existing_https_proxy)
        return False
...
def deregister_from_update_infrastructure():
    ...
    if not user:
        if not is_new_registration():
            log.debug('No credentials, nothing to do server side')
        return
    auth_creds = HTTPBasicAuth(user, password)
    if os.path.exists(smt_data_file):
        smt = get_smt_from_store(smt_data_file)
        smt_ips = (smt.get_ipv4(), smt.get_ipv6())
        log.debug('Clean current registration server: %s' % str(smt_ips))
        ...
        try:
            ...
            if response.status_code == 204:
                ...
                log.debug(rmt_check_msg)
        except requests.exceptions.RequestException as issue:
            log.warning(
                'Unable to remove client registration from server: {}'.format(
                    issue
                )
            )
        return server_name
...
def deregister_from_SCC():
    ...
    auth_creds = HTTPBasicAuth(user, password)
    if is_scc_connected():
        log.debug('Removing system from SCC')
        try:
            response = requests.delete(
                'https://scc.suse.com/connect/systems', auth=auth_creds
            )
            if response.status_code == 204:
                log.debug('System successfully removed from SCC')
            else:
...
def get_suma_registry_content():
    ...
    log.debug('Could not %s %s' % (action, SUMA_REGISTRY_CONF_PATH))
    return {}, True
    ...
    for smt_server_name in smt_server_names:
        for repo_file in repo_files:
            ...
            for section in repo_cfg.sections():
                ...
                if url:
                    if (
                        smt_server_name in url
                        or 'plugin:/susecloud' in url
                        or 'plugin:susecloud' in url
                    ):
...
def _get_region_server_args(plugin):
    ...
    while retry_cnt < 5:
        ...
        if not region_srv_args:
            ...
            time.sleep(1)
            continue
        log.debug('Region server arguments: ?%s' % region_srv_args)
        break
...
def _get_service_plugins():
    ...
    for service_plugin in service_plugins:
        if (
            os.path.basename(str(Path(service_plugin).resolve()))
...
def _remove_credentials(smt_server_names):
    ...
    log.debug('Deleting locally stored credentials')
...
def _write_suma_conf(updated_content):
    ...
    try:
        ...
        log.debug('{} updated'.format(SUMA_REGISTRY_CONF_PATH))
        return True
    except IOError as error:
        log.debug(str(error))
        action = 'open'
